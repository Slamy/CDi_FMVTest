...
int sigCode;
{
	...
	else if (sigCode == SIG_BLANK)
	{
		...
		static int sigpos = 201;

		if (input1State & (I_DOWN | I_UP))
		{
			dc_wrli(videoPath, lctB, sigpos * 2, 7, cp_nop());

			if (input1State & I_DOWN)
				sigpos++;
			else
				sigpos--;
			...
			/* The output might have disturbed the runtime here. Don't continue */
