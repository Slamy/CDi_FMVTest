...
void collect_registers(int pos)
{
	int V_UpdFlag = *(unsigned char *)(((char *)fdrvs1_static) + 0x12e);
	int V_VCMD = *(unsigned short *)(((char *)fdrvs1_static) + 0x16c);
	int V_Scroll = *(unsigned short *)(((char *)fdrvs1_static) + 0x16a);
	int V_Window = *(unsigned long *)(((char *)fdrvs1_static) + 0xe6);
	int V_DecOff = *(unsigned long *)(((char *)fdrvs1_static) + 0xea);
	int V_ScrOrg = *(unsigned long *)(((char *)fdrvs1_static) + 0xee);
	int V_ScrOff = *(unsigned long *)(((char *)fdrvs1_static) + 0xf2);
	int V_Status =  *(unsigned short *)(((char *)fdrvs1_static) + 0x136);

	regdump[regdump_index][0] = V_UpdFlag;
	regdump[regdump_index][1] = V_VCMD;
	regdump[regdump_index][2] = V_Scroll;
	regdump[regdump_index][3] = V_Window;
	regdump[regdump_index][4] = V_DecOff;
	regdump[regdump_index][5] = V_ScrOrg;
	regdump[regdump_index][6] = V_ScrOff;

	regdump[regdump_index][7] = FMV_SCRPOS;
	regdump[regdump_index][8] = FMV_DECOFF;
	regdump[regdump_index][9] = FMV_DECWIN;
	regdump[regdump_index][10] = FMV_GEN_DEC_CMD;
	regdump[regdump_index][11] = V_Status;

	regdump[regdump_index][12] = pos;
...
int sigCode;
{
	...
	else if (sigCode == SIG_BLANK)
	{
		...
		if (mpegStatus == MPP_PLAY)
		{
			...
			if (blankcount >= 14 && blankcount < 150)
			{
