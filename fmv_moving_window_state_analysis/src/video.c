...
int size;
{
    ...
    dc_nop(videoPath, lct, 0, 0, size,
           8); /* Fill all lines and cols of LCT with NOP instructions */
    return lct;
}
...
void setupPlaneA() {
    ...
    fctBuffer[i++] = cp_icm(
        ICM_CLUT7, ICM_CLUT7, NM_1, EV_ON,
        CS_A); /* Use CLUT7 for plane A and B, 1 Matte, External Video On */
    fctBuffer[i++] =
        cp_tci(MIX_OFF, TR_CKEY_T,
               TR_CKEY_T); /* Transparancy Color Key for Plane A and B */
    fctBuffer[i++] = cp_po(PR_AB); /* Plane A in front of B */
    fctBuffer[i++] =
        cp_bkcol(BK_BLACK, BK_LOW); /* Backdrop Low Intensity Black */
    fctBuffer[i++] =
        cp_tcol(PA, 0, 0, 0); /* Set transparancy color to black: rgb(0,0,0) */
    fctBuffer[i++] =
        cp_mcol(PA, 0, 0, 0); /* Set mask color to black: rgb(0,0,0) */
    fctBuffer[i++] = cp_yuv(PA, 16, 128, 128); /* Set DYUV start value */
    fctBuffer[i++] =
        cp_phld(PA, PH_OFF, 1); /* Set Mosaic (pixel_hold) off, size = 1 */
    fctBuffer[i++] = cp_icf(PA, ICF_MAX); /* Min Image Contributing Factor */
    fctBuffer[i++] = cp_matte(0, MO_END, MF_MF0, ICF_MAX, 0);
    fctBuffer[i++] =
        cp_dprm(RMS_NORMAL, PRF_X2, BP_NORMAL); /* Reload Display Parameters */
...
void setupPlaneB() {
    ...
    fctBuffer[i++] = cp_nop();
    fctBuffer[i++] =
        cp_tcol(PB, 0, 0, 0); /* Set transparancy color to black: rgb(0,0,0) */
    fctBuffer[i++] =
        cp_mcol(PB, 0, 0, 0); /* Set mask color to black: rgb(0,0,0) */
    fctBuffer[i++] = cp_yuv(PB, 16, 128, 128); /* Set DYUV start value */
    fctBuffer[i++] =
        cp_phld(PB, PH_OFF, 1); /* Set Mosaic (pixel_hold) off, size = 1 */
...
void initVideo() {
    ...
    videoMode = findstr(1, devParam, "LI=\"625\":")
                    ...
                    : (findstr(1, devParam, "TV")
                           ? 1
                           : 2); /* First parameter is first character to start
                                    searching at; 1-based, not 0-based! */
    printf("Video: %s %d\n", devParam, videoMode);
